...
{
  return Base::Singleton<Organya>::get();
}
...
{
  song.cur_beat = pos;
  if (song.cur_beat < 0) song.cur_beat = 0;
}
...
void Song::Synth()
{
  ...
  for (auto &i : ins)
  {
    ...
    for (int p = 0; p < n; ++p)
    {
      ...
      if (0)//settings->music_interpolation == 1)
      {
      ...
      else if (1)//settings->music_interpolation == 2)
      {
...
bool Organya::ff()
{
  ...
  _setPlayPosition(song.cur_beat + 5000 / song.ms_per_beat);
  return true;
}
...
bool Organya::rwd()
{
  ...
  if ( ((int)song.cur_beat - (int)(5000 / song.ms_per_beat)) < 0)
    _setPlayPosition(0);
  else
    _setPlayPosition(song.cur_beat - 5000 / song.ms_per_beat);
  return true;
