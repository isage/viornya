...
namespace natural
{
	...
	************************************************************************/
	...
	template<>
	inline bool compare(std::filesystem::directory_entry const &first ,std::filesystem::directory_entry const &second)
	{
		char* it1 = (char*)first.path().c_str();
		while(*it1!='\0')it1++;
		char* it2 = (char*)second.path().c_str();
		while(*it2!='\0')it2++;
		return _compare<char,char*>((char*)first.path().c_str(),it1,(char*)second.path().c_str(),it2);
	}
